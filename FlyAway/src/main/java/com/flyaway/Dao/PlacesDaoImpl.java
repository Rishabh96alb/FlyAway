...
public class PlacesDaoImpl implements PlacesDao {
	...
	public void update(Places place, int id) {
		...
		try {
			...
			Query<Places> query = session.createQuery(
					"update com.flyaway.entities.Places p set p.placeCode=?1, p.placename=?2 where p.placeId=?3");
	...
	public Places get(String placename) {
		...
		try {
			...
			session = factory.openSession();
			tx = session.beginTransaction();
			Query<Places> query = session
					.createQuery("select p from com.flyaway.entities.Places p where p.placeName=?1");
