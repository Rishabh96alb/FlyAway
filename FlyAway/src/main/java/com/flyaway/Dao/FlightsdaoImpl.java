...
public class FlightsdaoImpl implements FlightsDao {
	...
	public void update(int id, Flights flight) {
		...
		try {
			...
			Query<Flights> query = session.createQuery(
					"update com.flyaway.entities.Flights f set f.airline=?1, f.route=?2, f.price=?3, f.source=?4, f.destination=?5 where f.flightId=?6");
	...
	public List<Flights> getFlight(String source, String Destination) {
		...
		try {
			...
			session = factory.openSession();
			tx = session.beginTransaction();
			Query<Flights> query = session
					.createQuery("select f from com.flyaway.entities.Flights f where f.source=?1 and f.destination=?2");
	...
	public Flights getById(int id) {
		...
		try {
			...
			tx = session.beginTransaction();
			Query<Flights> query = session
					.createQuery("select f from com.flyaway.entities.Flights f where f.flightId=?1");
